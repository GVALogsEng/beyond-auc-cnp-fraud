...
## 2. Prior work this project uses
 
Three of the four items below are not background reading — they are components this project runs on: a formula, a scoring convention, and a benchmark's operating point. The fourth is a precedent we deliberately work against. Each is explained here once and used without re-explanation for the rest of the paper.

**The threshold formula — Elkan (2001). Used in §4.4, tested in §5.3.** A model returns a score for each transaction, but something still has to "turn" that score into approve or decline. The usual practice is to search for the cutoff: try many candidate values on held-out data and keep whichever scores best on some chosen metric. Elkan showed the search is unnecessary [2]. Write down what each of the two mistakes costs — approving a fraud, declining a good order — and the cost-minimizing cutoff follows from those two numbers by arithmetic. Nothing is tuned and nothing is fitted; if the business revises what a false decline is worth, the new cutoff is one line of algebra away. The result carries one condition, and that condition organizes the rest of this paper: the rule compares a probability against the cutoff, so the numbers a model emits have to be probabilities in the literal sense, not merely scores that happen to land between 0 and 1. Which is the next item.

**Reading scores as probabilities — Platt; Zadrozny & Elkan; Guo et al. Used in §4.4, measured in §5.2 and §5.4.** A model's scores are calibrated when they can be read as frequencies: among all transactions the model scores 0.20, roughly one in five should actually turn out to be fraud. This is a stricter requirement than ranking well, and it does not come for free — a model can order transactions flawlessly while emitting numbers that are uniformly too high or too low, because ordering is what most training procedures optimize and the scale is left to fall where it may.

When the scale is off, it can be repaired after the fact by fitting a correction on held-out data: a function mapping the model's raw score to the fraud rate actually observed at that score. Two standard corrections. Platt scaling fits a two-parameter S-shaped curve — stable on small samples, but it assumes the distortion has that shape [12a]. Isotonic regression fits a staircase whose only constraint is that it never steps downward — no assumption about shape, but it needs more data to fit [12b]. Both are monotone, meaning neither ever reorders two transactions, so ranking performance is untouched by construction and only the numbers move. Guo et al., studying modern neural networks, found miscalibration to be the rule rather than the exception and corrections of this kind sufficient to repair it [12c]; §5.2 confirms the same pattern for the weighted gradient boosting used here.

Fraud adds two complications. Training with class weights — which this project does, since fraud is 3.5% of rows — deliberately inflates the rare class's influence on the loss, distorting the probability scale as a side effect. And calibration decays: a correction fitted on April's data is fitted to April's mix of customers and attacks. Both are reasons to measure calibration on future data rather than assume it, which we do weekly using two standard quantities — the Brier score, the mean squared difference between predicted probability and outcome, lower being better; and expected calibration error, the average gap between predicted probability and observed fraud rate within bands of similar scores.

**The savings measure — Bahnsen, Aouada & Ottersten. The unit of every dollar figure in §5.** Elkan's formula prices a single decision; something still has to price a whole policy. Bahnsen and coauthors adapt cost-sensitive evaluation to card fraud, where the cost of a mistake belongs to the individual transaction rather than to the error type — approving a $2,000 fraud loses far more than approving a $12 one, so counting errors treats unlike things alike [10]. Their savings measure works by comparison: total the cost the model's decisions incur, compare it against the cheapest policy that uses no model at all — approve everything, or decline everything, whichever is cheaper on that data — and report the fraction of that cost removed. Zero means the model earned nothing over doing nothing. One means it removed all cost. Negative means it made matters worse. The denominator is the point: it fixes an honest zero, so a model cannot look impressive merely because fraud is rare.

**A fixed operating point — Jesus et al., Bank Account Fraud benchmark (NeurIPS 2022). Adopted as the headline ranking metric.** Two models can only be compared at equal strictness. The BAF benchmark standardizes this for imbalanced, drifting tabular data by fixing the strictness and asking what each model catches there: "we select the threshold in order to obtain 5% false positive rate (FPR), and measure the true positive rate (TPR) at that point" [11]. In plain terms, tune each model's cutoff until it wrongly flags 5% of legitimate transactions, then ask what share of the real fraud it caught. This mirrors the actual constraint on a fraud team: review capacity is finite, that capacity sets a budget for how many good customers may be interrupted, and models compete on how much fraud they catch inside it. We report TPR at 5% FPR throughout, with the dollar-denominated decision layer layered on top.

**The precedent we work against — Deotte & Yakovlev, IEEE-CIS winning solution. Not a method used here; the reason for §4.3's constraints and §5.1's measurement.** The remaining reference is not a tool. The competition's winning entry reconstructed a client identifier from card1, addr1, and D1, built features summarizing each client's behavior, and validated by holding out whole months rather than random rows — GroupKFold with month as the group, so every fold was scored on a stretch of time it had never trained on [8].

They validated that way because of the failure it blocks. Features summarizing a client's history let a model recognize that client instead of learning what fraud looks like, and a random split scatters a single card's transactions across both sides of the split, so recognition scores exactly as if it were generalization. The result is a number that is real on a leaderboard and unavailable in production, where the cards arriving tomorrow are largely cards the model has not seen. Section 5.1 measures the size of that effect directly. Two constraints in §4.3 exist to avoid it: our aggregate features use only strictly earlier transactions, and none of them touches the fraud label. The second is operational — in production the label is a chargeback arriving weeks after the transaction, so a feature like "this card's past fraud rate" quietly assumes information that would not exist at decision time.
