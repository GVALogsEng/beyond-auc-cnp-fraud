...
This section details the pipeline we use from raw stream to priced decisions. Four published components are applied in this, each introduced where it is used: a fixed operating point from the Bank Account Fraud benchmark (§3.2), two standard calibration repairs (§3.5), Elkan's cost threshold (§3.6), and the savings convention of Bahnsen et al. (§3.7). The intuitive breakdown of this section is a four-step process: class-weighted training distorts the probability scale on purpose (§3.4), calibration repairs it (§3.5), a threshold derived from error costs consumes the repaired probabilities (§3.6), and a dollar-denominated convention prices the resulting policy (§3.7). Sections 3.1–3.3 set up the protocol, the metrics, and the features that the process stands on.
